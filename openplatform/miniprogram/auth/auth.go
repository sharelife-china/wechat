...
// NewAuth new auth
func NewAuth(ctx *context.Context, miniAppID string) *Auth {
	return &Auth{miniAppID, ctx}
}
...
// Code2Session 登录凭证校验。
func (auth *Auth) Code2Session(jsCode, componentAccessToken string) (result ResCode2Session, err error) {
	return auth.Code2SessionContext(context2.Background(), jsCode, componentAccessToken)
}

// Code2SessionContext 登录凭证校验。
func (auth *Auth) Code2SessionContext(ctx context2.Context, jsCode, componentAccessToken string) (result ResCode2Session, err error) {
	var response []byte
	if response, err = util.HTTPGetContext(ctx, fmt.Sprintf(code2SessionURL, auth.miniAppID, jsCode, auth.AppID, componentAccessToken)); err != nil {
		return
	}
